...
<p style="font-family: 'Helvetica Neue', 'Helvetica', Helvetica, Arial, sans-serif; font-size: 14px; line-height: 1.6; font-weight: normal; margin: 0 0 10px; padding: 0;">
    <?= Yii::t('app', 'Congratulations! Your offer for order <b>"{0} ({1}, <span>{3}<span> - {2}, {4})"</b> has just been accepted', 
    [$order->title, $order->from_city, $order->to_city, strtoupper($order->from_country_short), strtoupper($order->to_country_short)]) ?>. 
    <?= Yii::t('app', 'Order has been moved to <b>To Realize</b> tab') ?>.
</p>
<p>
    <?= Yii::t('app', 'Click following link to see the order')?> 
    <a href="<?= Yii::$app->params['frontendUrl'] ?>/company/order/<?= $order->id?>"><?=Yii::$app->params['frontendUrl'] ?>/company/order/<?= $order->id?></a>.
</p>
<p style="font-family: 'Helvetica Neue', 'Helvetica', Helvetica, Arial, sans-serif; font-size: 14px; line-height: 1.6; font-weight: normal; margin: 0 0 10px; padding: 0;">
